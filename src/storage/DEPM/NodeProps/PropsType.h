...
namespace OpenEPL::DEPM {
     ...
     */
    template<typename Type> struct Props {
    ...
    public:
         ...
         */
        explicit Props() = default;

        /**
         * @brief Constructs a new NodeProps object with the given name.
         * @param name The name of the node.
         */
        explicit Props<Type>(const Key &name) : name(const_cast<Key &>(name)) {}
         ...
         */
        friend std::ostream &operator<<(std::ostream &os, const Props<Type> &node) {
            return os;
        }
    };
}
