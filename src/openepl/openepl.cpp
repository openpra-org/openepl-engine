...
#include <openepl/api/quantifier.h>
#include <openepl/utility/Helpers.h>
...
static void buildInputArguments(boost::program_options::options_description &generics) {
    generics.add_options()
            ...
            ("step-range,s", boost::program_options::value<std::string>()->default_value("0:10:100")->notifier(
                    [&](const std::string &value) {
                        static const std::regex step_range_pattern("^(0|[1-9][0-9]*):[1-9][0-9]*:(0|[1-9][0-9]*)$");
                        if (!std::regex_match(value, step_range_pattern)) {
                            throw boost::program_options::validation_error(
                                    boost::program_options::validation_error::invalid_option_value, "step-range", value);
                        }
                    }), "= Time-step range in the form 0:10:100")
            ;
}
...
static void run(nlohmann::json &resultJSON, boost::program_options::variables_map &map) {
    ...
    const auto stepRange = map["step-range"].as<std::string>();

    resultJSON = {
            {"model_filepath", inputFilePath},
            {"output",         {
                                       {
                                               "property", {
                                                                   {"method", methodName},
    ...
    if(methodName == "compute_MTTF") {
        resultJSON["output"]["result"] = OpenEPL::api::time::mttf(inputFilePath, targetTerm);
    }

    else if(methodName == "compute_P_single") {
        resultJSON["output"]["result"] = OpenEPL::api::probability::steadystate(inputFilePath, targetTerm);
    }

    else if(methodName == "compute_P") {
        const auto timeStep = static_cast<unsigned long>(asNumber(stepRange));
        resultJSON["output"]["result"] = OpenEPL::api::probability::at(inputFilePath, targetTerm, timeStep);
    }

    else if(methodName == "compute_Property") {
        resultJSON["output"]["result"] = OpenEPL::api::checkProperty(inputFilePath, targetTerm);
    }
}
